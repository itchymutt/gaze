...
Research conducted April 2026. The question: is anyone doing what Gaze proposes?
...
|---|---|---|---|---|---|
| **Gaze** | Us | Yes (type system) | Yes (10 effects) | Yes | Specification |
...
This is the strongest external validation of Gaze's thesis. Go's constraints weren't designed for agents, but they accidentally serve agents. One way to format code means agents don't waste tokens on style decisions. Rich introspection tools (`go doc`, `gopls`) give agents the context they need without reading source files. Structural simplicity means fewer ways to write the same thing, so agents produce more consistent output.

Gaze's constraints *are* designed for agents. The effect system, the fixed vocabulary, `gaze audit`, `gaze fmt` — these are the Go pattern made intentional. If Go accidentally became agent-friendly through constraint, a language designed from the start for agent-generated code should be dramatically more so.

Hashimoto also describes Go + Zig as a productive pairing: Go for high-level concurrent code, Zig for zero-dependency cross-compiled performance-critical paths. This is the two-language pattern Gaze competes against. Gaze's bet is that one language with effect tracking, value semantics, and Perceus RC can cover both roles. If it can't — if the high-level orchestration code and the performance-critical inner loops still want different languages — the Go+Zig approach wins by default.

### Zig: The Performance Proof

Zig is being used for inference in production AI systems (K2.6). Hashimoto: "If you want absolute performance with exacting control over what your CPU executes and the way memory is laid out, Zig is the way." Zig's allocation discipline — no hidden allocations, explicit memory layout, `comptime` for zero-cost abstractions — is the performance standard Gaze's native compilation (Phase 3) must eventually match.

Gaze doesn't need to beat Zig at memory layout control. That's not the target audience. But Gaze does need to be fast enough that the Go+Zig two-language pattern isn't strictly necessary. If a Gaze program with `can Unsafe` for the hot path can match "Go for the easy parts, Zig for the fast parts," the single-language story holds.


### Deno: The Runtime Proof

Deno's permission system is the closest existing thing to Gaze's effect vocabulary. Seven categories: read, write, net, env, run, ffi, sys. Enforced at process startup via CLI flags. Can be scoped to specific paths, domains, or variables.
...
Deno's limitation: runtime enforcement. A Deno program that calls `fetch()` without `--allow-net` crashes at runtime. A Gaze program that calls `net.get()` without `can Net` fails at compile time. The guarantee is strictly stronger.

Deno's Permission Broker protocol (2025-2026) externalizes permission decisions to an external process via JSON-RPC. This is relevant for libgaze: the effect manifest format should be compatible with or inspired by Deno's protocol.
...
A Gaze program compiled to WASM could auto-generate its Spin manifest from its effect declarations. The compile-time guarantee (Gaze) and the runtime enforcement (Spin/WASM) would be complementary layers.

### OWASP: The Market Demand

OWASP's Top 10 for LLM Applications (v2025) names the problems Gaze solves:

- **LLM07: Insecure Plugin Design.** "LLM plugins processing untrusted inputs and having insufficient access control risk severe exploits." Gaze's capability narrowing is the access control mechanism.
- **LLM08: Excessive Agency.** "Granting LLMs unchecked autonomy to take action can lead to unintended consequences." Gaze's effect system is the check on autonomy.

OWASP creates the demand. Gaze is the supply.
...
This is not a competitor (Rust's effects are about polymorphism, not restriction), but it normalizes effect vocabulary in mainstream programming. If Rust ships keyword generics, it validates Gaze's thesis. The risk: if Rust's initiative expands to include I/O capability tracking, it could occupy Gaze's space with the weight of Rust's ecosystem behind it.
...
No published papers exist on "effect systems for AI-generated code" (2024-2026). The PL theory community studies effect systems. The AI safety community studies sandboxing. Nobody has connected them formally. A paper on Gaze's approach would be novel and citable.

## What Nobody Is Doing

1. **Compile-time effect checking with a fixed vocabulary.** Every academic effect system is open/extensible. Gaze's fixed vocabulary is a deliberate constraint that enables standardization.
...
3. **Connecting PL effect theory to AI safety practice.** The two communities don't talk to each other. Gaze sits at the intersection.

## Allies, Not Competitors

- **Deno**: Validates the fixed-vocabulary model. Potential protocol compatibility.
- **Spin/WASM**: Complementary runtime enforcement. Auto-generated manifests from Gaze types.
- **OWASP**: Creates market demand for what Gaze supplies.
- **Koka/Effekt**: Academic credibility. Gaze can cite their work as theoretical foundation.
- **Rust effects initiative**: Normalizes effect vocabulary in mainstream programming.

## Threats

1. **Deno adds static checking.** If Deno's TypeScript integration gains compile-time permission verification, it occupies Gaze's space with an existing ecosystem. Probability: low (Deno's team is focused on runtime, not type theory).

2. **A major AI lab ships an effect protocol.** If OpenAI or Anthropic defines a standard for "what this code can do," it could become the de facto vocabulary before Gaze establishes its own. Probability: medium (the labs are focused on model capabilities, not code safety, but this could change).
