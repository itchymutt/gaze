...
**What they want from libgaze:** A library that does static analysis of agent-generated code and produces an effect manifest (Net, Fs, Db, etc.) that can be compared against a policy before execution. Pre-screening, not runtime enforcement (they already have that).
...
**What they want from libgaze:** Effect annotations on tool definitions. A decorator that says `@effects(Net, Fs.Read)` on a tool function, and libgaze verifies the implementation matches. Automatic effect propagation through tool chains.
...
**What they want from libgaze:** A CI step that analyzes AI-generated code diffs and reports the effect signature. "This PR adds Net access to a module that previously had none." The effect diff as a security signal. If a PR from Devin introduces `Unsafe` or `Db` effects in a frontend component, flag it.
...
**What they want from libgaze:** Policy-as-code for effects. A `.gazepolicy` file that says "services in /payments/ must not have effects: [Net.External, Unsafe]" and libgaze checks every PR against it.
...
**What they want from Gaze:** A real language with a real effect system that solves a real problem. Not another research language. Not another "we'll get to production eventually." A language they can write real programs in.
...
1. **libgaze CLI for Python/JS** (personas 1, 2, 3): `libgaze check agent_code.py` produces an effect report. No new language. No code changes. Works on existing agent code.

2. **libgaze CI integration** (personas 3, 4): GitHub Action that checks effect policies on every PR. `.gazepolicy` files checked into repos. Effect diffs in PR comments.

3. **Gaze the language** (persona 5, then all): By this point the vocabulary is battle-tested. The language is "what if your compiler enforced these effects natively?"
...
- **OWASP LLM07/LLM08** formally categorize the risks Gaze addresses
...
1. **E2B** -- They sandbox 500M+ agent executions. They need pre-execution screening. libgaze effect manifests are the missing layer between "the agent generated code" and "we ran it in a Firecracker VM."

2. **LangChain** -- 100M+ monthly downloads. They need effect annotations for tool definitions. libgaze decorators on tool functions would be a natural integration.
...
4. **Docker** -- The Dash platform already has compartments, capability grants, and egress policies. libgaze effect manifests map directly to Dash's permission model.

5. **Poolside** -- Their enterprise pitch explicitly includes "Executive-grade governance" and "Risk controls and auditability." libgaze is the technical implementation of what they're promising.
