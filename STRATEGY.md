...
Gaze enters a world with established programming languages, growing AI agent ecosystems, and no standard for describing what code does to the world. This document is about how to win that game.
...
If this vocabulary becomes the standard way to describe code behavior, Gaze wins regardless of adoption. A TypeScript project that annotates functions with `@can(Net, Db)` is using Gaze's vocabulary. A Python linter that checks `# effects: Fs, Fail` comments is using Gaze's vocabulary. An AI agent protocol that includes an effect manifest in every code submission is using Gaze's vocabulary.

The vocabulary is the platform. The language is one implementation of the platform. libgaze (the effect checker as a library) is the distribution mechanism.

This is the HCL pattern. HCL is a configuration language that became the standard for infrastructure-as-code. It succeeded not because everyone switched to HCL, but because the concepts it introduced (declarative infrastructure, plan-then-apply, human-readable-and-machine-parseable) became the standard way to think about infrastructure. Gaze's effect vocabulary should do the same for code behavior.
...
The effect manifest is this pattern applied to code generation. The manifest is the demonstration set — it declares what the code should do. The agent generates code (the completion). `gaze check` verifies the completion matches the declaration (the accuracy test). If the code's actual effects don't match the declared effects, it doesn't compile. The accuracy is 100% or it fails. This is prompt engineering with a compiler as the test harness.

The trust protocol has three components:
- **The vocabulary:** ten effects that describe what code does
- **The manifest:** a machine-readable declaration of every function's effects (the output of `gaze audit`)
...
Ship the smallest useful thing first. Not "the effect checker for all languages." Ship `libgaze` for Python — the analyzer that already exists, with a CLI (`libgaze check agent.py`), a policy engine (`.gazepolicy` files), and JSON manifest output. That's the `libghostty-vt` equivalent: the smallest piece extracted from the larger vision, zero dependencies, immediately useful, widely portable.
...
Then `libgaze` for JavaScript. Then the Rust core library. Then the protocol spec. Each ships independently. Each grows the vocabulary's adoption.

libgaze enters a positive-sum game. It doesn't compete with existing languages. It makes them better. Every language that adopts libgaze grows the market for effect-aware programming, which makes Gaze (the native implementation) more attractive.
...
An AI agent orchestrator built in Gaze. It uses effect manifests as its security model:
...
The full Gaze compiler, standard library, package manager, and ecosystem. By this point, the vocabulary is established, the library is adopted, the protocol is standardized, and the killer app has proven the thesis. The language is the capstone, not the foundation.
...
This is the economic argument for libgaze-first.

**Gaze the language is the application.** It requires adoption, learning, migration. It competes with every other language for mindshare. It's a high-quality mainline product that has to weigh every feature against every other feature.

**libgaze is the building block.** It's a pip-installable effect checker that agents can call. It doesn't require learning Gaze. It doesn't require migration. It makes existing Python and JavaScript code safer. An agent can grab it off the shelf, run `libgaze check agent.py`, and get an effect manifest. No adoption decision needed.

The building block economy has a second implication: **libgaze must be open. Unambiguously.** Hashimoto's observation, backed by independent research: models pick open and free software over closed and commercial. An effect checker that agents can't freely use is an effect checker that agents won't use. The vocabulary spreads through the building block. The building block spreads through openness.

The language can have a more nuanced licensing story. The library cannot. libgaze is open source, permissively licensed, or it doesn't become the standard.

## What We're Betting On

1. **AI agents will write most code within five years.** If this is wrong, Gaze is a nice language with an effect system. If this is right, Gaze is infrastructure.
...
4. **The library-first strategy works.** Shipping libgaze before the language is complete lets us enter the positive-sum game early. The risk: the library without the language is less compelling. The mitigation: the language specification and examples exist as proof of concept.

## What Could Kill This

1. **An existing language adds effect tracking.** If Rust ships an effect system RFC, or TypeScript adds effect annotations, the "no existing solution" claim dies. Mitigation: move fast on libgaze. Be the standard before someone else is.
